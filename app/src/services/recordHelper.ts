...
export const groupDailyRecords = (records: RecordItem[]): GroupedDailyRecords[] => {
  const groupedDailyRecords = groupBy(records, (record) => formatTimeStamp(record.timeStamp));
  return orderBy(
...
export const getDailySummaryInMonth = (records: RecordItem[], month: Moment): DailySummary[] => {
